    ...
    **/
    ...
    public function get_item_sizes($item_id,$shop_id)
    {
        ...
        if(isset($query->is_size) && $query->is_size==1):
            return isJson($query->price)?json_decode($query->price):'';
        else:
            return '';
        endif;
...
public function get_order_slug_by_slug($id)
{
 ...
 public function get_order_details_by_ids($ids,$shop_id)
    ...
    public function get_staff_orders_activities($staff_id,$type='total')
    {
       ...
       $this->db->where('o.is_draft',0);
       if($type=='today'){
        $this->db->where("DATE_FORMAT(o.created_at, '%Y-%m-%d') = '$today' OR DATE_FORMAT(o.completed_time, '%Y-%m-%d') = '$today'");
       }
       if($type=='lastMonth'){
    ...
    public function get_staff_activities__()
    {
        $this->db->select('o.*');
        $this->db->from('order_user_list o');
        $this->db->join("staff_list s",'s.id = o.staff_id',"RIGHT");
        $this->db->where('o.staff_id is NOT NULL', NULL, FALSE);
        $this->db->where('o.staff_action is NOT NULL', NULL, FALSE);
        $this->db->where('s.role','staff');
        $this->db->where('s.user_id',auth('id'));
    ...
    public function get_staff_activities($uid)
    {
        $this->db->select('u.*,s.name as staff_name');
        $this->db->from('order_user_list u');
        ...
        if(!empty($_GET['daterange'])){
            ...
            $sql = "(DATE_FORMAT(u.completed_time,'%Y-%m-%d') BETWEEN '{$newDate1}' AND '{$newDate2}') OR DATE_FORMAT(u.created_at,'%Y-%m-%d') BETWEEN '{$newDate1}' AND '{$newDate2}'";
            $this->db->where($sql);
        }

        $this->db->where('s.user_id',auth('id'));
        $this->db->where('s.uid',$uid);
        $this->db->order_by('u.accept_time, u.completed_time',"DESC");
        $query = $this->db->get();
        $query = $query->result();
        return $query;
    }
