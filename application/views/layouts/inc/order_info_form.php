	 ...
	 <div class="step_1">
	  	<span class="reg_msg"></span>
		  	...
		  	<?php if(isset($guest->is_guest_login) && $guest->is_guest_login==1): ?>
			  	<?php if(isset($guest->is_dine_in) && $guest->is_dine_in==1 && !empty(temp('is_table'))): ?>
	  		...
	  		<div class="form-group col-md-6">
	  			...
	  			<select name="order_type" id="order_type_list" class="form-control order_type <?= !empty(temp('is_table'))?"pointerEvent":"";?>" data-id="<?=$shop_id ;?>">
	  				...
	  				<?php foreach ($order_type as $key => $types): ?>
	  					<option value="<?=  $types['type_id'];?>" data-radius="<?= isset($shop_info['is_radius'])?$shop_info['is_radius']:0;?>" data-required="<?= $types['is_required'];?>"  data-pay="<?= $types['is_payment'];?>" data-slug="<?=  $types['slug'];?>" <?= !empty(temp('is_table'))&& $types['slug']=="dine-in"?"selected":"";?>><?= !empty(lang($types['slug']))?lang($types['slug']):$types['type_name'];?></option>
	  				<?php endforeach ?>
	  			</select>
	  		</div>
								...
								<select name="table_no" class="form-control table_no" id="table_no_1" data-id="<?= $shop_id;?>">
									<?php foreach ($dinein as $key => $dine): ?>
										<?php if(!empty(temp('table_no')) && temp('table_no')==$dine['id']): ?>
											<option value="<?= $dine['id'];?>" data-size="<?=  $dine['size'];?>" <?= !empty(temp('table_no')) && temp('table_no')==$dine['id']?"selected":"";?>><?=  $dine['name'];?> / <?= $dine['area_name'];?> - <?= $dine['size'].' '.lang('person');?> </option>
										<?php endif ?>
									<?php endforeach ?>
								</select>
							<?php else: ?>
											...
											<?php if(!empty($tipsValue->tips)): ?>
												<li>
													<label class="tipsLabel">
														...
														<input type="radio" name="tips_btn" value="<?= get_percent($this->cart->total(),$tipsValue->tips);?>">
													</label>
												</li>
											<?php endif; ?>
										<?php endforeach; ?>
