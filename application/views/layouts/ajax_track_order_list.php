						...
						<table class="table table-bordered table-condensed table-striped" id="example1">
							...
							<?php foreach ($order['item_list'] as $key => $row): ?>
										...
										<td>
										 ...
										 <div>
										 	<?php if($row['is_size']==1): ?>
											 	<label class="label bg-info-light-active ml-5"> <?= isset($row['size_slug'])? variants($row['size_slug'],$row['item_id'],$row['shop_id']):'';?></label>
											<?php endif;?>
										 </div>
										</td>
								...
								<?php 
									...
									$grand_total = grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type'],shop($order['shop_id'])->tax_status,$order['is_pos']);
								?>
							<?php endforeach ?>
							<tr class="bolder" >
								<td colspan="3"></td >
