		...
		<ul class="dropdown-menu dropdown-ul" role="menu">
			...
			<?php if($row['status'] == 0): ?>
				<?php if(is_access('order-cancel')==1): ?>
					<li class="cl-warning-soft" ><a href="javascript:;" data-shop="<?= $row['shop_id'] ;?>" class="rejectModal"data-url="<?= base_url('admin/restaurant/order_status/'.$row['uid']).'/3' ;?>" data-ajax='0' title="Mark as Cancel"><i class="icofont-hand-drag1"></i> &nbsp; <?= lang('cancel'); ?></span> </a></li>
				<?php endif; ?>
			<?php endif;?>
