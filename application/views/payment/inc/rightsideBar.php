			...
			<?php $i=1; foreach ($this->cart->contents() as $key => $row): ?>
				...
				<li>
					<div class="pay_size_top">
						<div class="">
							<span ><?= html_escape($row['qty']);?></span> x <span ><?= html_escape($row['name']) ;?> <span class="bg-primary-soft badge sizeText"><?= isset($row['sizes']['size_slug'])? variants($row['sizes']['size_slug'],$row['item_id'],$row['shop_id']):'';?></span> </span>
						</div>
						<div class="sizes">
